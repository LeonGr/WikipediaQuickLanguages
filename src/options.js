...
function showUnsupportedLanguages(selectedLanguages) {
    let unsupportedLanguages = [];
    for (const selectedLanguage of selectedLanguages) {
        if (!supportedLanguages.includes(selectedLanguage)) {
            unsupportedLanguages.push(selectedLanguage);
        }
    }

    if (unsupportedLanguages.length > 0) {
        document.querySelector("#unsupported-languages").innerText = `Unsupported languages: ${unsupportedLanguages}`;
    }
}
...
function saveOptions(e) {
    ...
    showUnsupportedLanguages(selectedLanguages);
...
function restoreOptions() {
    function setCurrentChoice(result) {
        if (result.selectedLanguages) {
            showUnsupportedLanguages(result.selectedLanguages);
